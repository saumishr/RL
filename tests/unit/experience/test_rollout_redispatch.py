...
class _ScriptedImpl:
    ...
    async def run_rollout(self, input_sample, *, rollout_group_id: str | None = None):
        # rollout_group_id is the per-attempt TQ identity and deliberately varies,
        # so it is not what these tests assert on; the recorded samples carry the
        # cohort identity, which is what has to stay stable across attempts.
